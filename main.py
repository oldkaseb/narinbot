...
MAIN_MENU_TEXT = "لطفاً یکی از گزینه‌ها را انتخاب کنید:"
...
def contains_malek(text: str) -> bool:
    ...
    return "نارین" in t  # شامل حالت‌های «مالکش/مالکشو/...»
...
async def cmd_setvserv(m: Message, state: FSMContext):
    ...
    await m.answer("لطفاً متن «خدمات مجازی» را ارسال کنید (HTML مجاز است). لغو: /cancel")
...
async def cmd_help(m: Message):
    ...
    admin_help = (
        ...
        "— /groupsend : پیام همگانی به همه گروه‌های ثبت‌شده.\n"
        "— /listgroups : مشاهده لیست گروه‌های ثبت‌شده.\n"
        "— /stats : آمار کاربران و گروه‌ها.\n"
        "— /addadmin <id> و /deladmin <id> : افزودن/حذف ادمین.\n"
        "— /block <id> و /unblock <id> : بلاک/آنبلاک کاربر.\n"
